...
import com.onthi.v_edu.ai.dto.ChatMessageRequest;
...
public class AiChatController {
    ...
    public ResponseEntity<ApiResponse<String>> chat(@RequestBody ChatMessageRequest request, @AuthenticationPrincipal UserDetails userDetails) {
        User user = userService.findByUsername(userDetails.getUsername())
                .orElseThrow(() -> new RuntimeException("User not found"));
        try {
            String response = aiChatService.chat(user, request.getMessage(), request.getSessionId());
    ...
    @GetMapping("/sessions")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiResponse<List<AiChatSession>>> getSessions(@AuthenticationPrincipal UserDetails userDetails) {
        User user = userService.findByUsername(userDetails.getUsername())
                .orElseThrow(() -> new RuntimeException("User not found"));
        return ResponseEntity.ok(new ApiResponse<>(HttpStatus.OK.value(), "Danh sách phiên chat", aiChatService.getUserSessions(user)));
    }
